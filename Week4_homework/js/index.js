//LAB GOALS
// - Create a chatbot that can ask a question and respond to 5 different user answers
// - Ask a follow-up question to at least 2 different user answers
// - Create a method called sendMessage, that takes 2 properties:
//      - type = "incoming" or "outgoing"
//      - message = "a string that is the message"
//      - move $('#convseration').append() into the sendMessage method
//      - everywhere you see $('#conversation').append() in the code now,
//      - should invoke this.sendMessage() and pass the correct parameters

// BONUS: can you get pictures and names for you and your chatbot?

// BONUS: can you use setTimeout() - Google it!
// Use it inside chatbot.ask() to make the chatbot seem more natural

// BONUS: could you use userAnswer.value.toLowerCase().search() - Google it!
// So that "I want pizza please" would match "pizza"?

// DOUBLE BONUS: Can you make it look like the chatbot is typing?
// Use in conjunction with setTimeout() above
// Hint: can use CSS class OR can add/remove an <li> based on an id

// DOUBLE BONUS: Can you use $('#conversation')[0].scrollHeight & $('#conversation').scrollTop()
// This will make the chat box act more like a true message window
// Hint: You'll need to put eveything in a container and work with a fixed height

//On load
$(function(){
    //Object
    var chatbot = {

            //#### PROPERTIES ####
            greeting: "Welcome to the sorting hat.",
            questions: {
                ask: 'Would you characteize yourself as brave, hardworking, smart, devious, or not magical?',
                //you can add:
                answers:[
                    {
                        value: 'brave',
                        ask:'how brave?',
                        answers:[
                          {
                            value:"very",
                             ask:"are you sure?",
                           },
                          {
                            value: "not very",
                            ask: "Would you consider not being a Gryffindor?"
                         }
                        ]
                    },
                    {
                      value: 'hardworking',
                      ask: 'Do you like being the fourth best at everything you try?',
                      answers: [
                        {value: "yes", ask: "Have you considered joining Hufflepuff?"},
                        {value: "no", ask: "Would youn consider not being a Huffelpuff?"}
                      ]
                    },
                    {
                      value: 'smart',
                      ask: 'Are you a fan of the color purple?',
                      answers: [
                        {value: "yes", ask: "Have you considered joining Ravenclaw?"},
                        {value: "no", ask: "Would you consider not being a Ravenclaw?"}
                      ]
                    },
                    {
                      value: 'devious',
                      ask: 'are you evil?',
                      answers: [
                        {value:"yes", ask:"Why are you evil?"},
                        {value:"no", ask:"Do you belong in Slytherin?"}
                      ]
                    },
                    {
                      value:'not magical',
                      ask:'are you a muggle?',
                      answers: [
                        {value: "yes", ask: "Eww gross!"},
                        {value: "no", ask: "should you be at hogwarts?"}
                      ]
                    }
                ]
            },
            answers: [],
            goodbye: "Great! Proceed to be sorted.",
            isOver: false,

    //FIRST Draft of Chatbot Resonses

    // var chatbot = {

        //#### PROPERTIES ####
        // greeting: "Welcome to the sorting hat.",
        // questions: {
        //     ask: 'Would you characteize yourself as brave, hardworking, smart, devious, not magical?',
        //     //you can add:
        //     answers:[
        //         {
        //             value: 'brave',
        //             ask:'how brave?',
        //             answers:[
        //               {
        //                 value:"very",
        //                  ask:"are you sure?",
        //                },
        //               {
        //                 value: "not very",
        //                ask:"Do you want to be here?"
        //
        //              }
        //             ]
        //         },
        //         {
        //           value: 'hardworking',
        //           ask: 'hufflepuff!'
        //         },
        //         {
        //           value: 'smart',
        //           ask: 'ravenclaw!'
        //         },
        //
        //         {
        //           value: 'devious',
        //           ask: 'are you evil?',
        //           answers: [
        //             {value:"yes", ask:"Why are you evil?"},
        //             {value:"no", ask:"Do you belong in Slytherin?"}
        //
        //           ]
        //         },
        //         {
        //           value:'not magical',
        //           ask:'are you a muggle?',
        //           answers: [
        //             {value: "yes", ask: "Eww gross!"},
        //             {value: "no", ask: "should you be at hogwarts?"}
        //           ]
        //         }
        //     ]
        // },
        // answers: [],
        // goodbye: "Great! Proceed to be sorted.",
        // isOver: false,

//END... first draft of chatbot responses....
        //#### METHODS ####
        // - Create a method called sendMessage, that takes 2 properties:
        //      - type = "incoming" or "outgoing"
        //      - message = "a string that is the message"
        //      - move $('#convseration').append() into the sendMessage method
        //      - everywhere you see $('#conversation').append() in the code now,
        //      - should invoke this.sendMessage() and pass the correct parameters

        sendMessage: function(type, message){
          if(type === "incomming"){
            $('#conversation').append('<li class="list-group-item text-left">' + message + '</li>');
          } else {
            $('#conversation').append('<li class="list-group-item text-right">' + message + '</li>');
          };
        },
        start: function() {
            this.ask(this.greeting);
            this.ask(this.questions.ask);
        },

        //message method
        ask: function(message) {

            if(message){
              this.sendMessage("incomming", message);
            }
        },


        answer: function() {
            var answer = $('#message').val();
            if(answer)
            {
                var answerObject = { value: answer };
                this.answers.push(answerObject); //add object to array
                this.sendMessage("outgoing", answer); //outgoign method
            }
            $('#message').val('');

            var question = this.nextQuestion();
            if(question)
            {
                this.ask(question);
            } else {
                this.end();
            }
        },
        nextQuestion: function() {
            //Default is the first question object
            question = this.questions;

            //Go through each of the USERS answers
            for(userAnswerIndex in this.answers)
            {
                var userAnswer = this.answers[userAnswerIndex];

                //Are there still unanswered questions?
                if(question && question.answers)
                {
                    //Iterate the answers to see if the user's answer matches any
                    for(index in question.answers)
                    {
                        var currentAnswer = question.answers[index];

                        if(userAnswer.value.toLowerCase() == currentAnswer.value.toLowerCase())
                        {
                            //Yay! the question is answered
                            question = currentAnswer;
                            break;
                        }
                    }

                //Otherwise there are no more questions unanswered
                } else {
                    return false;
                }
            }

            //This means we reached the end of the users answers and we still have a question that needs answering :)
            if(question)
            {
                return question.ask;
            } else {
                return false;
            }
        },
        end: function() {
            if(!this.isOver)
            {
                this.ask(this.goodbye);
                this.isOver = true;
                console.log(this.answers);
            }
        },

    //Attempted setTimeout, keep getting syntax error?

        // setTimeout: function(){
        //     console.log(self.name + "blah blah");
        // }, 3000);
    }

    //Event Listeners

    $('#addMessage').on('click', function(){
        chatbot.answer();
    });

    $('#formMessage').on('submit', function(event){
        event.preventDefault();
        chatbot.answer();
    });

    //Invoke the start method
    chatbot.start();
});
